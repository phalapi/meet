...
![图3-1 DI在PhalApi静态类结构中的位置](http://7xiz2f.com1.z0.glb.clouddn.com//ch-3-uml-di.jpg)  
...
![图3-2 来自Autoloading Standard的截图](http://7xiz2f.com1.z0.glb.clouddn.com//ch-3-pear-map.png)  
...
![图3-3 在线接口列表文档的访问效果](http://7xiz2f.com1.z0.glb.clouddn.com//ch-3-list-all-apis.png)  
...
![图3-4 调整后的在线接口文档](http://7xiz2f.com1.z0.glb.clouddn.com//ch-3-list-all-apis-welcome-say.png)  
...
![图3-5 商品快照信息的在线接口详情文档](http://7xiz2f.com1.z0.glb.clouddn.com/ch-3-check-goods-snapshot.png)
...
接口文档的注释较多，开发人员可在进行项目开发时按需注释。但各部分的规范也是简单明了的。结合上面的访问效果和商品快照信息接口类这一示例，从上到下，依次讲解各部分的使用规范。    
 ...
 + **接口**  当前接口服务的service名称，即：```Class.Action```。  
...
```
 ...
 + **接口参数**  根据接口类配置的参数规则自动生成，即对应当前接口类```getRules()```方法中的返回。其中最后的“说明” 字段对应参数规则中的desc选项。可以配置多个参数规则。此外，配置文件./Config/app.php中的公共参数规则也会显示在此接口参数里。这里的参数规则是：     
```php
...
![图3-6 添加了异常情况后的效果](http://7xiz2f.com1.z0.glb.clouddn.com//ch-2-goods-snapshot-docs-exception.png)
...
也就是说，通过把```@exception```注解和```@return```注解移到类注释，可以添加全部函数成员都适用的注解。例如，Api_User类的全部接口都返回code字段，且都返回400和500异常，则可以：  
```php
<?php
/**
...
![图3-7 JAVA版SDK包运行后的效果截图](http://7xiz2f.com1.z0.glb.clouddn.com//ch-3-java-sdk.jpg)
...
![图3-8 phalapi-buildapp命令的使用说明](http://7xiz2f.com1.z0.glb.clouddn.com//ch-3-buildapp.png)  
...
![图3-9 phalapi-buildtest命令的使用说明](http://7xiz2f.com1.z0.glb.clouddn.com//ch-3-buildtest.png)  
...
![图3-10 phalapi-buildsqls命令的使用说明](http://7xiz2f.com1.z0.glb.clouddn.com//ch-3-buildtest.png)  
...
![图3-11 phalapi-buildcode命令的使用说明](http://7xiz2f.com1.z0.glb.clouddn.com//ch-3-phalapi-buildcode.jpg)  
...
![图3-12 新增的Stock接口类](http://7xiz2f.com1.z0.glb.clouddn.com//ch-3-stock-go.jpg)
...
![图3-13 七牛云存储扩展类库提供的接口服务](http://7xiz2f.com1.z0.glb.clouddn.com//ch-3-qiniu-cdn.jpg)   
...
![图3-14 Task_Runner充当的角色](http://7xiz2f.com1.z0.glb.clouddn.com/ch-3-task-1.jpg)  

图3-14 Task_Runner充当的角色  

然后，我们可以分另各自实现接口服务和MQ队列，而让这两者互不影响。  

![图3-15 桥接模式在Task扩展中的应用 ](http://7xiz2f.com1.z0.glb.clouddn.com/ch-3-task-2.jpg) 
...
![图3-16 添加适配器后的静态结构](http://7xiz2f.com1.z0.glb.clouddn.com/ch-3-task-3.jpg)  
...
![图3-17 细化调度方式后的静态结构](http://7xiz2f.com1.z0.glb.clouddn.com/ch-3-task-4.jpg)  
...
![图3-18 计划任务的时序图](http://7xiz2f.com1.z0.glb.clouddn.com/ch-3-task-5.jpg)  
...
![图3-19 发红包运行效果，消息中的氛围图来自网络](http://7xiz2f.com1.z0.glb.clouddn.com/ch-3-wechat-red-pack.jpg)  
...
![图3-20 Wechat扩展的静态类结构图](http://7xiz2f.com1.z0.glb.clouddn.com/ch-3-wechat-uml.jpg)  
...
> phprpc官网：[http://www.phprpc.org/](http://www.phprpc.org/)  
...
最后，就是返回结果格式的改变。在phprpc协议下，因为可以更轻松地获取接口返回的源数据，所以这里也同样不再通过字符串流式的序列返回，如原来的JSON或XML格式，而是直接返回接口的源数据 。如前面示例中，返回的是数组类型。这一点，需要特别注意。  
...
> GetOpt的Github项目地址为：[https://github.com/ulrichsg/getopt-php](https://github.com/ulrichsg/getopt-php)  
...
PhalApi的自动加载，使用的是PAER的命名规范，秉承的原则是：简单、统一、规范。使用下划线作为目录分割符，暂不使用命名空间的原因是为了保持简单性。凡是符合PEAR命名规范的目录，通过```PhalApi_Loader::addDirs($dirs)```添加挂靠后，都能实现自动加载，例如项目目录Shop。也可使用```PhalApi_Loader::loadFile($filePath)```手动加载单个文件。但需要注意的是初始化文件和入口文件的区别，谨防类未找到的错误。  

自动生成的在线文档，包括接口列表文档和接口详情文档。根据各自的注释规范编写注释后，便可通过浏览器实时访问浏览。注意需要关注opcache。  

根据接口查询语言DSL，客户端可以通过一句话来描述接口请求，并且可以选择对应的SDK进行快速开发。若PhalApi框架未提供所需开发语言的SDK包，也可根据接口查询语言的文法进行搭建，主要有创建、初始化、重置、参数设置和请求等操作。  

有四个有趣的脚本命令是值得学习的，因为自动化能快速提升项目的开发速度，它们分别是：创建新项目的phalapi-buildapp命令、生成单元测试代码骨架的phalapi-buildtest命令、生成创建数据库表SQL语句的phalapi-buildsqls命令、生成接口服务体系代码的phalapi-buildcode命令。  

PhalApi框架的一大特色，是提供了可重用的扩展类库，致力于与更多的开源项目一起提供企业级的解决方案。引入可重用的扩展类库，可像搭积木那样快速搭建接口服务。当需要使用扩展类库时，可以按安装、配置、使用的步骤顺序整合到项目。其中，我们也学习了几个常用的扩展类库的使用，以及其背后的核心设计。你也可以封装自己的扩展类库，提供给其他项目使用，或分享到社区，实现从微架构到扩展类库的演进。  

最后，我们还学习了除了默认的HTTP/HTTPS协议外，如何构建RESTful API、使用PHPRPC协议、利用SOAP搭建Web Services和创建命令行CLI项目。而这些强大的功能，都是可通过可重用的扩展类库轻松搭建。你也可以使用其他的协议来访问接口服务，例如使用socket进行通信。  

从上一章对基础内容的点学习，到这章对高级主题的线学习，我们已经探索了PhalApi的大部分内容。而正是这些专业的知识，构建了我们专业的PhalApi开发技能、技巧和技艺。自然而然，接下来我们将会进行面的学习，即——
